...
double simulate(int N, int rank, int total_ranks, double* rmse_out) {
    ...
    for (int t = 0; t < steps; t++) {
        ...
        if (rank > 0) {
            MPI_Isend(&u_current[0], 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &requests[request_count++]);
            MPI_Irecv(&left_boundary, 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &requests[request_count++]);
        }
        if (rank < total_ranks - 1) {
            MPI_Isend(&u_current[local_num - 1], 1, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &requests[request_count++]);
            MPI_Irecv(&right_boundary, 1, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &requests[request_count++]);
        }
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < num_runs; i++) {
        double rmse = 0.0;
        double time = simulate(N, rank, total_ranks, &rmse);
        if (rank == 0) {
            total_time += time;
            total_rmse += rmse;
        }
    ...
    if (rank == 0) {
        ...
        double average_rmse = total_rmse / num_runs;
        printf("Average elapsed time over %d runs: %.5f seconds\n", num_runs, average_time);
        printf("Average RMSE over %d runs: %.10f\n", num_runs, average_rmse);
    }
